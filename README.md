...
| El agente busca tokens de otro tenant, o intenta usar `az cli`/tokens manuales | Solo se hizo el login de GitHub (para Copilot); el login de Fabric/Entra **dentro de la app local** nunca ocurrió, o se hizo con la cuenta equivocada | Abre la URL local en el navegador manualmente, cierra sesión de cualquier cuenta activa, y vuelve a iniciar sesión explícitamente con la cuenta que tiene permisos sobre el modelo. Luego repite el prompt indicando el usuario explícito (ver Bloque 6) |
